...
export default function AdminLogin() {
  ...
  useEffect(() => {
    ...
    if (errorParam === "unauthorized") {
      setError("Access Denied: Your Google account is not whitelisted. Please log in with an authorized administrator email.");
    }
  }, [searchParams]);
        ...
        <div className="glass-panel p-8 md:p-10 rounded-3xl border border-white/10 shadow-[0_0_50px_rgba(59,130,246,0.1)] backdrop-blur-2xl bg-[#0B0F19]/80">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-muted-foreground text-sm mt-2 text-center">
              Authorized personnel only. Access whitelisted by owner email.
            </p>
          </div>
          ...
          <div className="space-y-4">
            <button
              ...
              className="w-full py-4 rounded-xl bg-white text-black font-bold uppercase tracking-widest hover:bg-gray-100 transition-all shadow-[0_0_20px_rgba(59,130,246,0.15)] flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed group text-sm cursor-pointer font-sans"
            >
          ...
          <div className="mt-8 text-center flex flex-col gap-4">
            ...
            <div className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold flex items-center justify-center gap-1.5 mt-2">
              <ShieldCheck className="w-3.5 h-3.5 text-[#3B82F6]" /> Secure Connection Powered by Clerk
            </div>
          </div>
        </div>
      </motion.div>
